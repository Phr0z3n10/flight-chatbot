...
st.markdown("""<h1 style='color: #1B4F8A;'>  A.R.I.A.  🛩️</h1><p style='color: #2E5FA3;'>Airport Realtime Information Assistant </p> """, unsafe_allow_html=True)
...
if st.session_state.airport and len(st.session_state.messages) == 0:
     st.session_state.messages.append({"role": "user", "content": f"The user is located near {st.session_state.airport}. Use this as their default airport context."}) 
...
if user_question:
    ...
    if flight_number:
        ...
        if airport_code:
            ...
            if airport_data:
                dynamic_system = system_prompt + f"\n\nAIRPORT FLIGHT DATA FROM AERODATABOX: {airport_data}"
